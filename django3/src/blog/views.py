...
#DetailView는 request외에 추가 매개변수를 사용해 추출할 객체를 찾음 ->pk변수 :id값 같은 고유값으로 객체를 찾을때 사용하는 변수
...
#posting: 글쓰기 페이지
#FormView : 특정 폼크래스를 다루는 기능이 구현된 제네릭뷰
#LoginRequiredMixin:사용자가 비로그인 상태로 이 뷰를 요청한 경우, 로그인 페이지로 이동해주는 클래스다
#뷰함수 -> 데코레이터,  뷰 클래스->XXXXMixin 클래스 상속
class Posting(LoginRequiredMixin,FormView):
    template_name = 'blog/posting.html'
    #해당 뷰클래스가 사용할 폼클래스 지정
    form_class = PostForm
    context_object_name = 'f'
    #is_valid()함수가 True를 반환한 뒤의 처리를 form_valid()에서 함 
    ...
    def form_valid(self, form):
        ...
        #데이터베이스에 바로저장하지 않고 연동된 모델클래스로 변환만 한다
        ...
        for f in self.request.FILES.getlist('files'):
            pf = PostFile()#객체를 생성하려고 하니까 ()로를 붙여준다
            pf.post = p
            pf.file = f
            pf.save()
        #완성된 글의 URL을 전달
        return HttpResponseRedirect(reverse('blog:detail',args=(p.id,))) #args ,있는 이유: 튜플원소하나여서
